steps:
  - name: 'gcr.io/cloud-builders/docker'
    args: [
      'build',
      '-t',
      'europe-west3-docker.pkg.dev/$PROJECT_ID/cx-agents-repo/cx-agents-api:latest',
      '.'
    ]
  ...
  - name: 'gcr.io/google.com/cloudsdktool/cloud-sdk'
    ...
    args:
      ...
      - 'deploy'
      - 'cx-agents-api'
      - '--image=europe-west3-docker.pkg.dev/$PROJECT_ID/cx-agents-repo/cx-agents-api:latest'
      - '--region=europe-west3'
      - '--platform=managed'
...
# Import user memory functions for CF recommendations
from memory_history.user_memory import save_user_interaction, get_cf_recommendations

ALL_TOOLS = get_tools_sync()

# Sync with Qdrant vector DB for semantic search
ALL_TOOLS.append(semantic_search_products)
...
def _llm_invoke_with_retry(llm, messages, config):
    ...
    logger.error("Anthropic still overloaded after %d retries — giving up", _OVERLOAD_MAX_RETRIES)
    raise last_exc
...
class ShoppingState(TypedDict):
    ...
    cf_recommendations: Optional[list[dict]]   # CF output from user_memory
    last_added_product: Optional[str]          # track last add_to_cart product
...
def _build_system_message(state: ShoppingState) -> SystemMessage:
    ...
    # Inject CF recommendations into the system prompt if available
    cf_recs    = state.get("cf_recommendations", [])
    cf_section = ""
    if cf_recs:
        items      = ", ".join([r["item"] for r in cf_recs[:3]])
...
def _trim_context(messages: list[BaseMessage]) -> list[BaseMessage]:
    ...
    # Final safety: drop leading ToolMessages
    while trimmed and isinstance(trimmed[0], ToolMessage):
        trimmed = trimmed[1:]
...
# ─────────────────────────────────────────────────────────────────────────────
# HELPER: CATEGORY EXTRACTION
# ─────────────────────────────────────────────────────────────────────────────

def _extract_category(text: str) -> str:
    """
    Extract a product category from user message text using keyword matching.
    Can be replaced with an LLM call for higher accuracy if needed.
    """
    categories = {
        "tyre":      "tyres",
        "tire":      "tyres",
...
def agent_node(state: ShoppingState, config: RunnableConfig) -> dict:
    """
    Main reasoning node.
    - Builds context-aware system prompt (including CF recommendations)
    - Trims context window
    - Calls LLM with tool binding
    - Tracks tokens
    """
    session_id = state.get("session_id", "?")
    ...
    # Debug: log tool call/result pairing
    tool_call_debug = []
    for i, msg in enumerate(all_msgs):
        if isinstance(msg, AIMessage) and msg.tool_calls:
            tool_call_debug.append(f"AI#{i}: {[tc['id'][:8] for tc in msg.tool_calls]}")
    ...
    # Emergency cleanup: detect and remove unmatched tool calls before sending to LLM
    unmatched_calls = {}
    pending_calls   = {}
    for msg in all_msgs:
        if isinstance(msg, AIMessage) and msg.tool_calls:
    ...
    if pending_calls:
        logger.error(
            "agent_node | session=%s | CRITICAL: unmatched tool calls detected: %s | applying emergency cleanup",
            session_id, list(pending_calls.keys()),
        )
        cleaned_msgs = []
        for msg in all_msgs:
            if isinstance(msg, AIMessage) and msg.tool_calls:
                ...
                if safe_tool_calls:
                    ...
                    cleaned_msgs.append(AIMessage(content=msg.content, id=msg.id))
            else:
                cleaned_msgs.append(msg)

        all_msgs = cleaned_msgs
        logger.warning(
            "agent_node | session=%s | emergency cleanup applied — removed %d unmatched tool calls",
            session_id, len(pending_calls),
        )
    ...
    for msg in reversed(state["messages"]):
        ...
        if not result.get("success"):
            ...
            if "CERTIFICATE_VERIFY_FAILED" in error_str or "SSL" in error_str.upper():
                logger.error(
                    "SSL_ERROR | context=tool_result | session=%s | tool_call_id=%s | error=%s",
                    session_id, msg.tool_call_id, error_str,
                )
                audit("API_ERROR", session_id, {
                    "error":       error_str,
                    "context":     "sap_tool_call",
...
def memory_node(state: ShoppingState) -> dict:
    ...
    Responsibilities:
    1. Save the user's interaction (search or add_to_cart) to Qdrant via user_memory.
    2. Generate CF recommendations based on similar users' behaviour.
    3. Inject CF recommendations into state so agent_node can use them next turn.
    """
    ...
    if not last_human:
        logger.debug("memory_node | session=%s | no human message found, skipping", session_id)
        return {}

    # Check for add_to_cart action in the most recent tool results
    for msg in reversed(messages):
        if isinstance(msg, ToolMessage):
            try:
                result = json.loads(msg.content)
                if result.get("success") and result.get("entry_number") is not None:
                    # An add_to_cart was performed — save the interaction
                    save_user_interaction(username, {
                        "action":       "add_to_cart",
                        "query":        last_human,
                        "product_code": state.get("last_added_product", ""),
                        "category":     _extract_category(last_human),
                    })
                    logger.debug(
                        "memory_node | session=%s | saved add_to_cart interaction for user=%s",
                        session_id, username,
                    )
            except Exception as e:
                logger.warning("memory_node | session=%s | failed to parse tool result: %s", session_id, e)
            break

    # Save search action if user was looking for something
    search_keywords = ["search", "show", "find", "looking", "want", "need", "get"]
    if any(word in last_human.lower() for word in search_keywords):
        try:
            save_user_interaction(username, {
                "action":   "search",
                "query":    last_human,
            ...
            logger.debug(
                "memory_node | session=%s | saved search interaction for user=%s",
                session_id, username,
            )
        except Exception as e:
            logger.warning("memory_node | session=%s | failed to save search interaction: %s", session_id, e)

    # Generate CF recommendations for next turn
    try:
        cf_recs = get_cf_recommendations(username, last_human, top_k=5)
        if cf_recs:
            logger.debug(
                "memory_node | session=%s | CF recommendations generated: %d items for user=%s",
                session_id, len(cf_recs), username,
            )
            return {"cf_recommendations": cf_recs}
    except Exception as e:
        logger.warning(
            "memory_node | session=%s | failed to get CF recommendations: %s",
            session_id, e,
        )
...
def tool_node_with_injection(state: ShoppingState) -> dict:
    ...
    # Create ToolMessage results for rejected (cancelled) tool calls
    tool_results = []
    for tool_call in last.tool_calls:
        if tool_call["id"] in rejected_tool_calls:
            tool_results.append(ToolMessage(
    ...
    # Patch access_token into non-rejected tool calls
    patched_calls = []
    for tc in last.tool_calls:
        if tc["id"] not in rejected_tool_calls:
            args = dict(tc.get("args", {}))
    ...
    # If all tool calls were rejected, return only rejection messages
    if not patched_calls:
        return {"messages": tool_results, "rejected_tool_calls": None}
...
def build_production_graph():
    ...
    g.add_node("tools",          tool_node_with_injection)
    g.add_node("sync",           state_sync_node)
    g.add_node("memory",         memory_node)           # NEW: memory + CF node
    ...
    # After sync: if more tool calls are pending, go back to agent
    # Otherwise, run memory node then end the turn
    g.add_conditional_edges("sync", route_after_sync, {
        "agent": "agent",
        END:     "memory",     # sync -> memory -> END
    })

    g.add_edge("memory", END)  # memory is always the final step
...
def new_session(user_id: str = "anonymous") -> tuple[ShoppingState, str]:
    ...
    if static_token:
        logger.warning(
            "new_session | Using SAP_STATIC_TOKEN — dev mode only | session=%s",
            thread_id,
        )
        access_token      = static_token
        resolved_user_id  = "current"
    ...
    init_state = ShoppingState(
        ...
        cf_recommendations=None,        # NEW
        last_added_product=None,        # NEW
    )
